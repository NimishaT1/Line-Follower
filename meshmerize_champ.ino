...
void GetSensorValues(){
  ...
  for (int i = 0; i<IR; i++){
    ...
    pastSensorVals[curHistoryPos++] = Sensors;
    curHistoryPos %= historySize;
...
bool checkPosition(char *str){
  ...
  for(int j = 0; j < 5; j++){
    for(int i = 0; i < historySize; i++){
      avgSensorVals[j] += pastSensorVals[(curHistoryPos+i)%historySize][j];
    }
    avgSensorVals[j] /= historySize;
  }
  bool match = true;
...
void explore(){
  ...
  if(checkPosition("1*1*") || checkPosition("*1*1")){
    ...
    int start = millis();
    int current = start;
    while(!checkPosition("0***0") && (current-start<stopTime)){
      current = millis();
      GetSensorValues();
      tempturn = getTurn();
    ...
    if (current-start>=stopTime && checkPosition("11111")){
      digitalWrite(TPin, 1);
      Running = false;
    } else if (turn==1){
      turnLeft();
